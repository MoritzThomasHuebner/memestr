...
# part = int(sys.argv[2])
# event_number = 0
time_tag = precessing_events[event_number].time_tag
event = precessing_events[event_number].name
detectors = precessing_events[event_number].detectors
# result = bilby.core.result.read_in_result(f'{event}/result/run_data0_{time_tag}_analysis_{detectors}_dynesty_par{part}_result.json')
result = bilby.core.result.read_in_result(f'{event}/result/run_data0_{time_tag}_analysis_{detectors}_dynesty_merge_result.json')
# result.outdir = f'{event}_{suffix}/result/'
result.outdir = f'{event}/result/'
...
wg_nrsur_memory = bilby.gw.waveform_generator.WaveformGenerator(
    sampling_frequency=ifos.sampling_frequency, duration=ifos.duration,
    frequency_domain_source_model=memestr.waveforms.fd_nr_sur_7dq4_with_memory)

bilby.core.utils.logger.disabled = False


likelihood_nrsur = bilby.gw.likelihood.GravitationalWaveTransient(
    interferometers=ifos, waveform_generator=wg_nrsur)
likelihood_nrsur_memory = bilby.gw.likelihood.GravitationalWaveTransient(
    interferometers=ifos, waveform_generator=wg_nrsur_memory)
...
try:
    ...
    reweighted_time_shift_memory_log_bf, log_memory_weights = memestr.postprocessing.reweigh_by_likelihood(
        new_likelihood=likelihood_nrsur_memory, result=result,
        reference_likelihood=likelihood_nrsur, use_stored_likelihood=True)
    np.savetxt(f"{event}_memory_log_weights", log_memory_weights)
