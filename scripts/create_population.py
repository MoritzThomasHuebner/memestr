    ...
    # while network_snr < 12:
        ...
        for ifo in ['H1', 'L1']:
            ...
            if ifo in ['H1', 'L1']:
                ...
                # interferometer_mem_ref.power_spectral_density = bilby.gw.detector.PowerSpectralDensity.from_aligo()
            else:
                ...
                #     from_power_spectral_density_file('AdV_psd.txt')
                # interferometer_mem_ref.power_spectral_density = bilby.gw.detector.PowerSpectralDensity.\
                #     from_power_spectral_density_file('AdV_psd.txt')
            ...
            #     duration=settings.waveform_data.duration,
            #     start_time=start_time)
            # interferometer_mem_ref.set_strain_data_from_power_spectral_density(
            #     sampling_frequency=settings.waveform_data.sampling_frequency,
            #     duration=settings.waveform_data.duration,
            #     start_time=start_time)
            ...
            # injection_polarizations_mem_ref = interferometer_mem.inject_signal(
            #     parameters=settings.injection_parameters.__dict__,
            #     injection_polarizations=hf_signal_mem_ref)

            # plt.plot(waveform_generator_memory.frequency_array,
            #          np.abs(hf_signal_mem['plus']))
            # plt.plot(waveform_generator_memory_ref.frequency_array,
            #          np.abs(hf_signal_mem_ref['plus']))
            # plt.loglog()
            # plt.xlim(10, 1024)
            # plt.show()
            # plt.clf()
            ...
            # signal_mem_ref = interferometer_mem.get_detector_response(
            #     injection_polarizations_mem_ref, settings.injection_parameters.__dict__)
            #
            ...
            # interferometer_mem_ref.plot_data(signal=signal_mem_ref, outdir='.', label=str(filename) + '_mem_ref')
        ...
        # best_mem_snrs = [ifo.meta_data['matched_filter_SNR'].real for ifo in ifos_mem]
