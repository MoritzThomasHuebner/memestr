...
def gamma(
        lm1: str, lm2: str, incs: np.ndarray = None, theta: np.ndarray = None,
        phi: np.ndarray = None, y_lmlm_factor: np.ndarray = None) -> list:
    """
    ...
    pol = 0
    incs = incs or np.linspace(0, np.pi, 500)
    theta = theta or np.linspace(0, np.pi, 250)
    phi = phi or np.linspace(0, 2 * np.pi, 500)
    y_lmlm_factor = y_lmlm_factor or _get_y_lmlm_factor(lm1=lm1, lm2=lm2, theta=theta, phi=phi)

    lambda_lm1_lm2 = np.array([lambda_lmlm(inc, pol, lm1, lm2, theta, phi, y_lmlm_factor) for inc in incs])

    sin_inc = np.sin(-incs)

    harm = {f"{l}{m}": harmonics.sYlm(-2, l, m, incs, pol) for l, m in harmonics.lmax_modes(20)}
    ...
    for ell in ells:
        if ell < abs(delta_m):
            ...
            result.append(
                np.trapz(lambda_lm1_lm2 * np.conjugate(harm[f"{ell}{delta_m}"]) * sin_inc, incs) * 2 * np.pi)

    return result


def _get_y_lmlm_factor(lm1: str, lm2: str, theta: np.ndarray, phi: np.ndarray) -> np.ndarray:
    s = -2
    ...
    return \
        harmonics.sYlm(ss=s, ll=l1, mm=m1, theta=th, phase=ph) * (-1) ** (l2 + m2) \
        * harmonics.sYlm(ss=-s, ll=l2, mm=-m2, theta=th, phase=ph)
...
def plus_tensor(wx, wy, wz=np.array([0, 0, 1])):
    ...
    return np.outer(wx, wx) - np.outer(wy, wy)
...
def cross_tensor(wx, wy, wz=np.array([0, 0, 1])):
    ...
    return np.outer(wx, wy) + np.outer(wy, wx)
